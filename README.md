...
Cross-scale validation used distributed pipeline parallelism to split 70B across 2 GPU workers (40 transformer blocks each). I used [Zagora](https://zagora.ai) for this step, but any distributed fine-tuning setup that supports LoRA on 70B-class models works since the discovered recipe is just hyperparameters.
...
2. **Lower rank wins under time budgets**: Rank 4 beats 8/16/32 because it's cheaper per step, and gives more optimizer steps in the same wall-clock time.
...
I used [Zagora](https://zagora.ai), a distributed LoRA fine-tuning platform using Petals pipeline parallelism, to split the 80 transformer blocks across 2 workers (40 blocks each) with a VPS coordinator. Any distributed fine-tuning tool capable of LoRA on 70B+ models can be used to reproduce this step.
...
# Single baseline run (5-min budget, 500 examples, default hyperparams)
python train.py

# Single run with discovered C2 recipe
...
### Phase 2: Confirmation (single GPU)

Each candidate and the baseline run 3× with seeds {42, 123, 777}, 2000 examples, and 10-minute budget.

```bash
# Baseline confirmation (repeat for --seed 123 and --seed 777)
python train.py --max_examples 2000 --time_budget 600 --seed 42

# C2 confirmation (repeat for --seed 123 and --seed 777)
python train.py --lr 1.7e-4 --rank 4 --alpha 8 --dropout 0 \
  --weight_decay 0 --warmup_ratio 0 --scheduler linear \
  --target_modules q_proj k_proj v_proj o_proj gate_proj up_proj down_proj \
  --max_examples 2000 --time_budget 600 --seed 42
```
